...
export function createClassroom(env) {
  ...
  const db = openStore(env.CLASSROOM_DB_PATH || resolve('data/classroom.sqlite'));
  ...
  const ready = (async () => {
    if (!db.prepare("SELECT id FROM users WHERE role = 'admin'").get()) {
      ...
      db.prepare('INSERT INTO users VALUES (?,?,?,?,?,?,?)').run(randomUUID(), adminLogin, '관리자', 'admin', hash, now(), null);
    }
  })();
  function limit(key, maximum = 25, duration = 15 * 60 * 1000) {
    const time = Date.now();
    db.prepare('DELETE FROM rate_limits WHERE expires_at < ?').run(time);
    db.prepare('INSERT INTO rate_limits VALUES (?,1,?) ON CONFLICT(key) DO UPDATE SET count=count+1').run(digest(key), time + duration);
    if (db.prepare('SELECT count FROM rate_limits WHERE key=?').get(digest(key)).count > maximum) throw fail(429, '요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.');
  }
  function session(req) {
    const token = req.headers.cookie?.split(';').map(x => x.trim()).find(x => x.startsWith(cookieName + '='))?.slice(cookieName.length + 1);
    if (!token || token.length > 100) return null;
    return db.prepare('SELECT u.* FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?').get(digest(token), Date.now()) || null;
  }
  ...
  function setSession(res, user) {
    ...
    db.prepare('INSERT INTO sessions VALUES (?,?,?)').run(digest(token), user.id, Date.now() + 12 * 60 * 60 * 1000);
  ...
  function events(userId) {
    return db.prepare('SELECT id,kind,payload,received_at FROM events WHERE user_id=? ORDER BY id').all(userId).map(e => ({ id: e.id, kind: e.kind, payload: JSON.parse(e.payload), at: e.received_at }));
  }
  function report(u, details = false) {
    const p = progress(u.id); const rows = events(u.id);
  ...
  async function handle(req, res) {
    try {
      ...
      if (route === '/health' && req.method === 'GET') { db.prepare('SELECT 1').get(); return reply(res, 200, { enabled: true, version: LESSON_VERSION }); }
      if (route === '/session' && req.method === 'GET') { const u = session(req); return reply(res, 200, { user: u ? safeUser(u) : null, version: LESSON_VERSION }); }
      if (['/login', '/activate', '/admin-login'].includes(route) && req.method === 'POST') {
        const body = await readBody(req, 4096);
        limit('auth-global', 600); limit('auth:' + String(body.studentId || body.login || '').slice(0,40));
        const login = route === '/admin-login' ? body.login : body.studentId;
        const u = typeof login === 'string' ? db.prepare('SELECT * FROM users WHERE student_id=?').get(login.trim()) : null;
        ...
        if (route === '/activate') {
          ...
          const update = db.prepare('UPDATE users SET password_hash=? WHERE id=? AND password_hash IS NULL').run(hashed, u.id);
        ...
        setSession(res, u); return reply(res, 200, { user: safeUser(u) });
      }
      ...
      if (route === '/progress' && req.method === 'GET') {
        ...
        return reply(res, 200, { ...progress(u.id), summary: report(u, true) });
      }
      if (route === '/sync' && req.method === 'POST') {
        if (u.role !== 'student') throw fail(403, '학생 계정에서만 저장합니다.');
        limit('sync:' + u.id, 180, 60 * 1000);
        ...
        try {
          const current = progress(u.id);
          const allSaved = cleanEvents.length > 0 && cleanEvents.every(e=>db.prepare('SELECT id FROM events WHERE user_id=? AND event_key=?').get(u.id,e.key));
          if (body.revision !== current.revision) {
            if (allSaved && JSON.stringify(current.state) === JSON.stringify(state)) { db.exec('ROLLBACK'); return reply(res, 200, current); }
            throw fail(409, '다른 창에서 기록이 바뀌었습니다. 새로고침해 최신 기록을 불러오세요.');
          }
          for (const e of cleanEvents) db.prepare('INSERT OR IGNORE INTO events (user_id,event_key,kind,payload,received_at) VALUES (?,?,?,?,?)').run(u.id,e.key,e.kind,JSON.stringify(e.payload),now());
          db.prepare('INSERT INTO progress VALUES (?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET state=excluded.state,revision=excluded.revision,updated_at=excluded.updated_at').run(u.id,JSON.stringify(state),current.revision+1,now());
          db.exec('COMMIT');
          return reply(res, 200, progress(u.id));
        } catch (error) { if (db.isTransaction) db.exec('ROLLBACK'); throw error; }
      }
      authenticate(req, 'admin');
      if (route === '/admin/students' && req.method === 'GET') {
        const students = db.prepare("SELECT * FROM users WHERE role='student' ORDER BY student_id").all().map(user => report(user));
        return reply(res, 200, { students, joinCode: env.CLASSROOM_JOIN_CODE });
      }
      const detail = route.match(/^\/admin\/students\/([\w-]+)$/);
      if (detail && req.method === 'GET') {
        const student = db.prepare("SELECT * FROM users WHERE id=? AND role='student'").get(detail[1]);
        if (!student) throw fail(404, '학생을 찾을 수 없습니다.');
        return reply(res, 200, report(student, true));
      }
      if (route === '/admin/roster' && req.method === 'POST') {
        ...
        let added = 0; db.exec('BEGIN IMMEDIATE');
        try {
          for (const entry of entries) {
            const existing = db.prepare('SELECT name,role FROM users WHERE student_id=?').get(entry.studentId);
            ...
            if (!existing) { db.prepare('INSERT INTO users VALUES (?,?,?,?,?,?,?)').run(randomUUID(),entry.studentId,entry.name,'student',null,now(),null); added++; }
          }
          db.exec('COMMIT');
      ...
      if (route === '/admin/export' && req.method === 'GET') {
        const students = db.prepare("SELECT * FROM users WHERE role='student' ORDER BY student_id").all();
        const rows = [['학번','이름','참여 상태','활동 완료(6개)','응답 문항(8개)','첫 응답 정답 수','최근 응답 정답 수','최근 로그인','최근 저장']];
        for (const student of students) { const r = report(student); rows.push([r.studentId,r.name,r.eventCount ? '학습 중' : r.lastLogin ? '로그인만 함' : '미참여',r.completed,r.answered,r.firstCorrect,r.correct,r.lastLogin,r.updatedAt]); }
  ...
  return { enabled, handle, close: () => db.close(), ready };
}

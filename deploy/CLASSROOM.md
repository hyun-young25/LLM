...
기존 QR의 GitHub Pages 기본 주소 `https://hyun-young25.github.io/LLM/`는 학생 로그인 `https://gpt-learning-classroom.onrender.com/#login`으로 자동 이동합니다. QR을 다시 만들 필요가 없습니다. 로그인·학습 기록·관리자 페이지는 같은 Vue 화면과 Node 서버를 한 주소에서 제공합니다. 학생 이름·학번·응답, 관리자 비밀번호와 데이터베이스를 GitHub에 커밋하지 마세요.
...
3. `ADMIN_PASSWORD`는 관리자 전용 비밀 환경변수입니다. 소스나 VITE 환경변수에 넣지 마세요. 수업 참여 코드는 사용하지 않으며, 기존 서버에 `CLASSROOM_JOIN_CODE` 값이 남아 있어도 학생 참여에는 영향을 주지 않습니다.
4. 배포 주소의 `/#admin`에서 `instructor`와 관리자 비밀번호로 로그인합니다. 전체 명단(학번,이름)을 미리 등록하는 것은 선택 사항입니다. 미리 등록하면 아직 한 번도 들어오지 않은 학생도 미참여로 비교할 수 있습니다. 동일 명단은 기존 기록을 유지하며 다른 이름과 충돌하면 명단 전체를 취소합니다.
5. 학생은 수업용 주소에서 이름·학번만 입력하고 **학습 시작**을 누릅니다. 처음 참여하면 명단에 자동 등록되며, 같은 이름·학번으로 다시 들어오면 이전 기록을 이어갑니다. 학생 비밀번호나 참여 코드, 별도 가입 절차는 없습니다. 학번에 이미 다른 이름이 등록되어 있으면 기존 정보를 덮어쓰지 않습니다.
...
무료 웹 서비스 `gpt-learning-classroom`과 무료 Postgres `gpt-learning-records`의 연결을 완료했습니다. 2026년 9월 11일(한국 시간) 운영 주소에서 데이터베이스 상태 확인, 관리자 로그인·학생 목록 조회·로그아웃, 비로그인 관리자 접근 차단을 확인했습니다. 기존 QR을 안내하면 학생이 이름·학번으로 참여할 수 있습니다. 새 Blueprint로 중복 생성하지 마세요.
...
학습 상태는 서버에 자동 저장하며 같은 이름·학번으로 다음 로그인 때 복원합니다. 문항 응답은 서버에서 정답을 재판정하고, 처음 고른 답과 이후 변경한 답을 별도 이벤트로 보존합니다. 문항은 형성평가용이므로 학습 화면과 공개 소스에 해설이 있습니다. 이름·학번 입력은 본인 확인 절차가 없는 간편 참여 방식입니다. 다른 사람이 같은 정보를 알면 해당 기록으로 접속할 수 있으므로, 이 기록만으로 신원을 확인하거나 출석·시험 성적을 확정하지 않습니다.
...
세션은 12시간 후 만료하며 로그아웃 시 폐기됩니다. 로그인과 쓰기 요청에 빈도 제한을 적용하고, 모든 관리자 API에서 역할을 검사합니다. 학생 간편 참여로 관리자 계정에 로그인할 수 없습니다. 관리자 비밀번호나 학습 기록은 브라우저 저장소에 보관하지 않습니다. 관리자 비밀번호는 salt가 있는 scrypt 해시로, 세션 토큰은 SHA-256 해시로 서버에 저장합니다. 신규 학생 계정에는 비밀번호를 저장하지 않으며, 이전 학생 계정의 기존 해시는 간편 참여에 사용하지 않습니다. 기존 계정 ID와 활동 기록은 유지합니다.
...
관리자 비밀번호 분실 시 서버 운영자가 `RESET_LOGIN`과 `RESET_PASSWORD`를 비밀 환경변수로 제공하고 `node deploy/reset-password.mjs`를 한 번 실행할 수 있습니다. 해당 계정의 기존 세션을 모두 폐기합니다. 무료 Render 웹 서비스는 Shell/SSH가 없으므로 이 명령은 데이터베이스에 안전하게 연결된 별도 관리 환경에서 실행해야 합니다. 실행 후 두 변수를 제거하세요. 계정과 학습 기록은 유지됩니다.

## 검증

`npm test`: 기존 수학·생성 계산과 별도로 이름·학번 자동 등록, 동시 첫 접속의 중복 계정 방지, 관리자 비밀번호·역할 보호, 기존 이름·학번 보존, CSRF 방어, 최초·최근 응답과 서버 채점, 중복 저장·동시 수정, 재시작 뒤 복구, CSV 수식 방어, 저장 실패 복구를 검증합니다. GitHub Actions는 별도의 PostgreSQL 17 테스트 데이터베이스에서도 동일한 API 시나리오를 실행합니다.

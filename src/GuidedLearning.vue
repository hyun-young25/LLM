...
<template>
  <section class="guided-learning" aria-label="따라하며 배우는 GPT 생성 원리">
    ...
    <div v-else-if="step === 2" class="guide-activity">
      <div class="guide-controls"><label for="sampleContext">문맥<select id="sampleContext" v-model="sampleContext"><option v-for="(context, key) in contexts" :key="key" :value="key">{{ context.label }}</option></select></label><label for="guideTemperature">Temperature <strong>{{ sampleTemperature.toFixed(1) }}</strong><input id="guideTemperature" v-model.number="sampleTemperature" type="range" min="0.2" max="1.6" step="0.1" /></label></div>
      <fieldset class="guide-reason"><legend>먼저 예상: Temperature를 높이면 어떻게 될까요?</legend><button type="button" :class="{ selected: samplePrediction === 0 }" :aria-pressed="samplePrediction === 0" @click="samplePrediction = 0">높은 확률의 후보에 더 집중된다</button><button type="button" :class="{ selected: samplePrediction === 1 }" :aria-pressed="samplePrediction === 1" @click="samplePrediction = 1">후보 사이의 확률 차이가 줄어든다</button></fieldset>
